...
public class RemnantRoadsStructure {
	...
	private static class FeatureRegisterHandler {
		...
		public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
					...
					if ((random.nextInt(1000000) + 1) <= 750) {
						...
						for (int a = 0; a < count; a++) {
							...
							int y = spawnTo.getY();
							int z = spawnTo.getZ();
							if (!OldVillageAdditionalGenerationConditionProcedure
									...
									random);
							{
								...
								RemnantRoadsOnGenerationProcedure.executeProcedure($_dependencies);
							}
						}
					}
					return true;
			...
			event.getRegistry().register(feature.setRegistryName("remnant_roads"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("back_in_classic:remnant_roads"), configuredFeature);
		}
	}
	@SubscribeEvent
